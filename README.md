...
explode() は区切り文字で分割する関数で、文字単位の分割には向きません
str_split() は文字列を指定した長さ（デフォルト1文字）で分割します
fgets() で読み取った文字列には改行が含まれるため、trim() で除去します
explode('', $input) です。explode() の第1引数に空文字列 '' を指定すると、PHP 8.1以降では警告/エラーになります。

また、空文字列では分割できません。
複数桁の数値を一桁の数字の配列にするには、str_split() を使います。
