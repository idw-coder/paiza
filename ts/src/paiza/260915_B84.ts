...
function solve(N: number, M: number, K: number, myList: number[], t: number[][]) {
  if (DEBUG)
    console.log(
      '\n\n' +
        N +
  ...
  const similarList = t.filter((shops) => {
    return (
      K <= shops.reduce((count, value, i) => count + (value === 3 && myList[i] === 3 ? 1 : 0), 0)
    )
  })
  if (DEBUG) console.log(similarList.join('\n'))
...
reader.on('close', () => {
  const [N, M, K] = lines[0]!.split(' ').map(Number) as [number, number, number]
