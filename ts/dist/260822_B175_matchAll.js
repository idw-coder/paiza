...
function solve(n, l, r, s) {
    ...
    for (const m of s.matchAll(/\d+|\D/g)) {
        if (/\D/.test(m[0]))
            ...
            if (pos < l && l < pos + num && pos + num < r) {
                if (DEBUG)
                    console.log(prev.repeat(pos + num - l + 1));
                out += prev.repeat(pos + num - l + 1);
            }
            else if (pos < l && r <= pos + num) {
                out += prev.repeat(r - l + 1);
                if (DEBUG)
                    console.log(prev.repeat(r - l + 1));
                break;
            }
            else if (l <= pos && pos + num < r) {
                if (DEBUG)
                    console.log(prev.repeat(num));
                out += prev.repeat(num);
            }
            else if (l < pos && r <= pos + num) {
                if (DEBUG)
                    console.log(prev.repeat(r - pos));
    ...
    if (DEBUG)
        console.log(JSON.stringify([...s.matchAll(/\d+|\D/g)].map((m) => m[0])));
    console.log(out);
    return;
}
...
//# sourceMappingURL=260822_B175_matchAll.js.map