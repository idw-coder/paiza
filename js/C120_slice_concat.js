...
reader.on('close', () => {
  ...
  flowerNum = parseInt(lines[0]);
  flowerArrayA = lines[1].split('');
  flowerArrayB = lines[2].split('');
  for (let i = 0; i < flowerNum; i++) {
    yesOrNo = "Yes";
    for (let k = 0; k < flowerNum; k++) {
        if (flowerArrayA[k] !== flowerArrayB[k]) {
            yesOrNo = "No";
            break;
        }
    }
    if (yesOrNo === "Yes") break;
    
    // 1つ左にローテーション
    flowerArrayA = flowerArrayA.slice(1).concat(flowerArrayA.slice(0, 1));
 }
