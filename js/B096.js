function calc(N, inputArray, calledArray) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let j = 0; j < 4; j++) {
      if (calledArray.includes(inputArray[i][j])) {
        remaining_numbers = remaining_numbers.filter((number) => number !== inputArray[i][j])
  ...
  // 縦のビンゴをチェック
  for (let i = 0; i < 4; i++) {
    logContent += '縦 ' + i + '列目 ' + inputArray.map((row) => row[i]).join(' ') + '\n'
    let remaining_numbers = inputArray.map((row) => row[i])
    for (let j = 0; j < 4; j++) {
      if (calledArray.includes(inputArray[j][i])) {
        remaining_numbers = remaining_numbers.filter((number) => number !== inputArray[j][i])
      }
    }
    if (remaining_numbers.length === 1) {
      logContent += 'リーチ　 ' + remaining_numbers[0] + '\n'
